...
- 非 agent 路径下按配置中的 identity 顺序尝试解密：先无 prompt 尝试明文 identity（age native 或未加密 SSH 私钥），再对 scrypt / 加密 SSH identity 按需 prompt passphrase（scrypt 在 rspass 侧提前 prompt；加密 SSH 经由 age callbacks 在 stanza 匹配时 prompt）
...
1. **明文 age identity 文件**（`AGE-SECRET-KEY-1…` 行 + 可选注释）— 直接按行解析
2. **scrypt 加密** — 整个文件是一个 age-encrypted blob，明文是上述 identity 文件；解锁需要 passphrase
3. **未加密的 SSH 私钥**（`~/.ssh/id_ed25519` 等 OpenSSH 私钥）— 由 `age::ssh::Identity::from_buffer` 解析
4. **加密的 SSH 私钥**（`ssh-keygen -N <pass> …` 生成，OpenSSH 默认的 `bcrypt-pbkdf + aes256-ctr` 格式）— 与未加密 SSH 走同一分类路径；解锁时机按下文两种上下文不同

分类方法：读取文件头 → `age-encryption.org/v1` 判为 scrypt；首个非空非 `#` 行匹配 `-----BEGIN … PRIVATE KEY-----` 判为 SSH；其它走 `age::IdentityFile`。

`agent ls` 对 SSH entries 不打印公钥（age crate 未暴露 pubkey accessor）。非 `bcrypt-pbkdf + aes256-ctr` 的 SSH 加密格式（罕见，通常是手工构造的 legacy 文件）会在 `ssh-key` crate 解析 / 解密阶段报错。

### 加密 SSH 私钥的解锁时机

分两种上下文：

- **本地回退**（`show` / `edit` 没走 agent 的路径）：加载时用 `age::ssh::Identity::from_buffer` 得到 `Identity::Encrypted(...)`，立即 `.with_callbacks(tty::TtyCallbacks)` 包装装入 identity 池。age crate 在真正 unwrap 到匹配 stanza 时通过 callback 调 `tty::prompt_passphrase`。Wrong-passphrase 不会在 callback 内循环（跟随 rage 的行为），解密失败就把这个 identity 当作不匹配处理，继续试下一个
- **`agent add`**：daemon 没有 tty，不能在解密时再 prompt。CLI 侧先用 `ssh-key` crate 解析，`is_encrypted()` 为真时进入跟 scrypt 同样的 prompt / 重试 / 空输入跳过 / Ctrl+C 取消循环；解密成功后 `.to_openssh(LineEnding::LF)` 序列化为**未加密** OpenSSH PEM 送给 daemon。daemon 始终只看到未加密 identity 材料
...
2. **agent 中没有的 on-disk identity**：
   - 明文 age identity / 未加密 SSH 私钥 → 直接读取
   - scrypt identity → 按 config 顺序逐个 prompt passphrase 并尝试
   - 加密 SSH 私钥 → 以 `age::ssh::Identity` + `TtyCallbacks` 形态装入池；age 在 stanza 匹配到它时才触发 passphrase prompt
